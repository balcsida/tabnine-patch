...
- A guard on the analytics `send` method so extension events no-op when the telemetry config has no `tabnineHost`, instead of throwing on every install.
...
- `findAnalyticsHostGuard` matches the analytics `send` method and adds a `this.config.tabnineHost &&` short-circuit so no-host telemetry configs skip the request rather than throwing.
